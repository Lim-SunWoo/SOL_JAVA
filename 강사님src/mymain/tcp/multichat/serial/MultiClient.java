...
public class MultiClient extends JFrame {
	...
	private void init_input() {
		...
		jbt_color.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(sel_color==null)return;//취소 누르면 끝내라
					...
					while(true) {
						...
						try {
							...
							if(data.protocol==Data.IN) {
								...
								String msg = String.format("->[%s]님 입장", data.nick_name);
								my_display_message(msg);
								
								
							}else if(data.protocol==Data.OUT) {
								
								String msg = String.format("<-[%s]님 퇴장", data.nick_name);
								my_display_message(msg);
								...
								my_display_user_list(data);
								
							}else if(data.protocol==Data.DRAW) {
								
									//메모리 그리기
									Graphics g = memPan.getGraphics();
									g.setColor(new Color(data.line_color));
									g.fillOval(data.x-data.thick, 
											   data.y-data.thick, 
											   data.thick*2, 
											   data.thick*2);
	...
	protected void my_display_user_list(Data data) {
		// TODO Auto-generated method stub
